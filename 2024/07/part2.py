...
class Node:
    ...
    def set_concat(self, value: int):
        val = int(str(self.value) + str(value))
        self.concat = Node(val)

    def is_leaf_matching_target_result(self, target_result: int) -> bool:
        if self.add is None and self.mul is None and self.concat is None:
        ...
        return (self.add.is_leaf_matching_target_result(target_result)
                ...
                or self.concat.is_leaf_matching_target_result(target_result))
...
def create_tree(node, numbers, index):
    try:
        ...
        node.set_concat(current_number)
        create_tree(node.concat, numbers, next_index)
    except:
        pass
